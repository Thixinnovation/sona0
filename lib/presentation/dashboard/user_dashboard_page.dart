...
class _EducationEditorBodyState extends State<_EducationEditorBody> {
  ...
  Future<void> _pickEvidenceFiles() async {
    try {
      final res = await FilePicker.platform.pickFiles(allowMultiple: true, withData: kIsWeb, type: FileType.custom, allowedExtensions: const ['pdf', 'png', 'jpg', 'jpeg', 'webp']);
      ...
      for (final f in res.files) {
        ...
        await _docs.uploadPickedFile(uid: uid, docId: docId, title: 'Pièce cursus: ${_degreeC.text.trim().isEmpty ? f.name : _degreeC.text.trim()}', file: f, docType: 'credential_education');
...
class _ProfileEditorBodyState extends State<_ProfileEditorBody> {
  ...
  Future<void> _pickIdFile(String kind) async {
    try {
      final res = await FilePicker.platform.pickFiles(type: FileType.custom, withData: kIsWeb, allowedExtensions: const ['png', 'jpg', 'jpeg', 'webp', 'pdf']);
      if (res == null || res.files.isEmpty) return;
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 10),
                        Expanded(child: Text('Pièces d\'identité (photo/PDF)', style: context.textStyles.labelLarge?.copyWith(fontWeight: FontWeight.w900))),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
